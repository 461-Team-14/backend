...
class TestDefaultController(BaseTestCase):
    ...
    def test_create_auth_token(self):
        ...
        response = self.client.open(
            '/KAMELH/ECE461/2.0.0/authenticate',
            method='PUT',
    ...
    def test_package_by_name_delete(self):
        ...
        response = self.client.open(
            '/KAMELH/ECE461/2.0.0/package/byName/{name}'.format(name=PackageName()),
            method='DELETE',
            headers=headers)
        self.assert200(response,
                       'Response body is : ' + response.data.decode('utf-8'))
    ...
    def test_package_by_reg_ex_get(self):
        ...
        headers = [('x_authorization', AuthenticationToken())]
        response = self.client.open(
            '/KAMELH/ECE461/2.0.0/package/byRegEx',
            method='POST',
            data=json.dumps(body),
            headers=headers,
            content_type='application/json')
    ...
    def test_package_delete(self):
        ...
        headers = [('x_authorization', AuthenticationToken())]
        response = self.client.open(
            '/KAMELH/ECE461/2.0.0/package/{id}'.format(id=PackageID()),
            method='DELETE',
            headers=headers)
        self.assert200(response,
                       'Response body is : ' + response.data.decode('utf-8'))
    ...
    def test_package_rate(self):
        ...
        headers = [('x_authorization', AuthenticationToken())]
        response = self.client.open(
            '/KAMELH/ECE461/2.0.0/package/{id}/rate'.format(id=PackageID()),
            method='GET',
            headers=headers)
        self.assert200(response,
                       'Response body is : ' + response.data.decode('utf-8'))
